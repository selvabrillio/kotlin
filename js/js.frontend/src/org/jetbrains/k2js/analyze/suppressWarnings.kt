...
class SuppressUnusedParameterForJsNative : DiagnosticsWithSuppression.SuppressStringProvider {
    override fun get(annotationDescriptor: AnnotationDescriptor): String? {
        val descriptor = DescriptorUtils.getClassDescriptorForType(annotationDescriptor.getType())
        if (FqNameUnsafe(PredefinedAnnotation.NATIVE.fqName) == DescriptorUtils.getFqName(descriptor)) {
            return Errors.UNUSED_PARAMETER.getName().toLowerCase()
        }
